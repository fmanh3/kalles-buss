...
describe('UC-FIN-01: Automatiserad Fakturering av Körtur', () => {
  ...
  beforeAll(async () => {
    ...
    await db.schema.createTable('ledger_transactions', (table: any) => {
      table.uuid('id').primary();
    ...
    billingEngine = new BillingEngine(db, ledgerService);
  });
  ...
  it('Scenario: Lyckad fakturering av avslutad tur', async () => {
    ...
    const ledgerEntries = await db('ledger_entries').where({ transaction_id: transaction.id });
