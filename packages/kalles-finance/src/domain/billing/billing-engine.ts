...
export class BillingEngine {
  ...
  async processTourCompletion(tour: TourCompletedEvent, apc: ApcStats) {
    ...
    await this.db.transaction(async (trx) => {
      ...
      // 4. Bokför i huvudboken via LedgerService
      await this.ledger.recordTransactionWithTrx(trx, {
        ...
        entries: [
          { account_code: '1510', debit: totalInclVat, credit: 0 }, // Kundreskontra
          { account_code: '3000', debit: 0, credit: totalExclVat }, // Försäljning
          { account_code: '2611', debit: 0, credit: vatAmount }    // Utgående moms 6%
        ]
      });
    });
