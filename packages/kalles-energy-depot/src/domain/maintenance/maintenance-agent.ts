...
export class MaintenanceAgent {
  ...
  constructor(private db: Knex) {
    this.pubsub = new PubSub({ projectId: 'joakim-hansson-lab' });
  }

  /**
   * UC-DEPOT-01: Behandla skaderapport och skapa arbetsorder.
   */
  async processDamageReport(report: DamageReport) {
   ...
   */
  private async triggerProcurement(itemId: string, assetId: string) {
